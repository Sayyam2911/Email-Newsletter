
const Benefits = () => {
  return (
    <div className="benefit-cover min-h-[90vh] relative flex items-center justify-center">
      <div className="w-[63%] md:h-[320px] bg-[#F7F5FF] border-[2px] rounded border-blue-500 p-2">
        <h3 className="mt-8 font-clashDisplay uppercase text-3xl md:text-6xl text-center w-full mb-4">
          EVERYTHING YOU NEED TO <span className="font-style">SUCCEED</span>{" "}
          AVAILABLE IN A SINGLE PLATFORM
        </h3>
      </div>
    </div>
  );
};

export default Benefits;
